...
### 3-1.js
- 브라우저에서 전역 컨텍스트에서 실행되는 this는 window 객체를 가리키므로 this === window는 true.

### 3-2.js
- Node.js의 전역 컨텍스트에서 this는 global 객체를 가리키므로 this === global은 true.
- 브라우저 환경의 window와 비슷한 개념.

## 3-3.js
- 브라우저 전역에서 `var`로 선언된 변수가 자동으로 `window` 객체의 프로퍼티가 됨.
- this === window이므로 this.a 또한 동일한 값 1을 반환함.

### 3-4.js
- var a = 1 → window.a = 1으로 자동 등록됨. window.b = 2로 직접 등록한 후, b로도 접근 가능함 → 전역 스코프에선 
  b = 2도 같은 효과.
- window.a = 3으로 바꾸면 a, this.a도 모두 반영됨. b = 4로 재할당하면 window.b, this.b도 동일하게 변경됨.
- 전역 변수와 window 객체 간의 동기화된 참조를 설명하고, var 없이 선언한 변수도 window에 등록됨.

### 3-5.js
- `delete` 연산자를 통해 전역 변수와 `window` 객체의 프로퍼티를 삭제할 수 있음.
- var로 선언된 변수(a, b)는 configurable: false 속성으로, delete로 제거할 수 없음.
- 반면 window.c = 3, window.d = 4처럼 직접 추가한 전역 프로퍼티는 configurable: true이므로 삭제 가능함.

### 3-6.js
- 함수를 단독으로 호출할 때와, 객체의 메서드로 호출할 때의 `this` 바인딩 차이 확인 가능

### 3-7.js
- 객체의 메서드를 점 표기법(.)과 대괄호 표기법([])으로 호출할 때, `this`가 동일하게 객체 자신을 가리킴.

### 3-8.js
- 이 예제는 중첩 객체 구조 내에서의 메서드 호출 시 `this`는 바로 앞 개체를 참조.
- 또한, 점 표기법(`.`)과 대괄호 표기법(`[]`)의 차이가 `this` 바인딩에는 영향을 주지 않음.

### 3-9.js
- 동일한 함수(`innerFunc`)를 어떻게 호출하느냐에 따라 `this`가 다르게 바인딩됨.
